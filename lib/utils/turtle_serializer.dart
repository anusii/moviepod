...
class TurtleSerializer {
  ...
  static String moviesToTurtle(List<Movie> movies, String listName) {
    ...
    for (final movie in movies) {
      ...
      buffer.writeln(
        '  movie:description "${_escapeString(movie.overview)}" ;',
      );
      buffer.writeln('  movie:image "${_escapeString(movie.posterUrl)}" ;');
      buffer.writeln(
        '  movie:thumbnailUrl "${_escapeString(movie.backdropUrl)}" ;',
      );
      buffer.writeln(
        '  movie:aggregateRating "${movie.voteAverage}"^^xsd:double ;',
      );
      buffer.writeln(
        '  movie:datePublished "${movie.releaseDate.toIso8601String()}"^^xsd:dateTime ;',
      );
