...
class PodFavoritesService extends ChangeNotifier {
  ...
  Future<void> _initializePodData() async {
    try {
      ...
      if (isPodReady) {
        ...
        debugPrint('POD not available, using empty data (files will be created when data is added)');
  ...
  Future<void> _loadFileFromPod(String fileName, Function(dynamic) onSuccess) async {
    int retryCount = 0;
    const maxRetries = 3;
    ...
    while (retryCount < maxRetries) {
      try {
        debugPrint('Attempting to load file from POD: $fileName (attempt ${retryCount + 1}/$maxRetries)');
        ...
        debugPrint('Ensuring encryption key is available for reading $fileName');
        await getKeyFromUserIfRequired(_context, const Text('Loading encryption key'));
        ...
        debugPrint('ReadPod returned content type: ${content.runtimeType}, length: ${content is String ? content.length : 'N/A'}');
        onSuccess(content);
        ...
        if (e.toString().contains('does not exist')) {
          debugPrint('File $fileName does not exist in POD yet (this is normal for new storage)');
          return; // Don't retry for non-existent files
        } else {
          debugPrint('Error loading $fileName from POD (attempt $retryCount/$maxRetries): $e');
  ...
  Future<void> _saveToWatchToPod(List<Movie> movies) async {
    ...
    try {
      ...
      debugPrint('Attempting to save ${movies.length} movies to POD: $_toWatchFileName');
      final ttlContent = TurtleSerializer.moviesToTurtleWithJson(movies, 'toWatchList');
      debugPrint('Generated TTL content length: ${ttlContent.length} characters');
      ...
      debugPrint('Ensuring encryption key is available for saving $_toWatchFileName');
      await getKeyFromUserIfRequired(_context, const Text('Loading encryption key'));
      debugPrint('Encryption key ready for saving');
      
      debugPrint('Writing to POD path: $_toWatchFileName');
      final result = await writePod(_toWatchFileName, ttlContent, _context, _child);
  ...
  Future<void> _saveWatchedToPod(List<Movie> movies) async {
    ...
    try {
      final ttlContent = TurtleSerializer.moviesToTurtleWithJson(movies, 'watchedList');
  ...
  Future<void> reloadFromPod() async {
    try {
      final isPodReady = await isPodAvailable();
      if (isPodReady) {
        debugPrint('Reloading data from POD after app folders initialization...');
        await _loadFromPod();
